...
const ProductRegistry = () => {
  ...
  const handleExport = () => {
    const dataToExport = filteredProducts.map(p => {
      const { margin, marginRate } = calculateMargin(p.sellingPrice, p.purchasePrice, p.isSalesVatIncl, p.isPurchaseVatIncl);
      return {
        'No.': p.no,
        '상품코드': p.id,
        '상품명': p.name,
  ...
  const calculateMargin = (selling: number, purchase: number, isSalesVatIncl: boolean = true, isPurchaseVatIncl: boolean = true) => {
    // If VAT is included, convert to base price (Net)
    const netSelling = isSalesVatIncl ? selling / 1.1 : selling;
    const netPurchase = isPurchaseVatIncl ? purchase / 1.1 : purchase;
    
    const margin = netSelling - netPurchase;
    const marginRate = netSelling > 0 ? (margin / netSelling) * 100 : 0;
          ...
          <table className="w-full text-left border-collapse min-w-[1000px]">
            ...
            <tbody className="divide-y divide-slate-50">
              {filteredProducts.map((product) => {
                const { margin, marginRate } = calculateMargin(product.sellingPrice, product.purchasePrice, product.isSalesVatIncl, product.isPurchaseVatIncl);
                return (
                  <tr key={product.id} className="hover:bg-slate-50/30 transition-colors text-[11px] group">
                    <td className="px-6 py-4 font-mono text-slate-300 group-hover:text-slate-600 text-center">{String(product.no).padStart(2, '0')}</td>
      ...
      <AnimatePresence>
        {isModalOpen && (
          <ModalWrapper
            ...
            title={editingProductId ? '상품 정보 수정' : '신규 기초 상품 등록'}
            icon={<Package size={20} />}
          >
            <form onSubmit={handleAddOrUpdateProduct} className="p-8 space-y-5 bg-white">
              <div className="space-y-1.5 text-left">
                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest pl-1">상품명 *</label>
                <input 
                  type="text" 
                  placeholder="상품명을 입력하세요"
                  className="w-full px-4 py-3 bg-slate-50 border-none rounded-xl text-sm font-medium focus:ring-2 focus:ring-[#2D336B]/20 transition-all outline-none shadow-inner"
                  value={formData.name}
              ...
              <div className="grid grid-cols-2 gap-4 text-left">
                <div className="space-y-1.5">
                  ...
                  <select 
                    className="w-full px-4 py-3 bg-slate-50 border-none rounded-xl text-sm font-bold focus:ring-2 focus:ring-[#2D336B]/20 outline-none"
                ...
                <div className="space-y-1.5">
                  <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest pl-1">규격</label>
                  <input 
                    type="text" 
                    className="w-full px-4 py-3 bg-slate-50 border-none rounded-xl text-sm font-medium focus:ring-2 focus:ring-[#234] outline-none shadow-inner"
              ...
              <div className="grid grid-cols-2 gap-4 text-left">
                <div className="space-y-1.5">
                  <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest pl-1">단위</label>
                  <input 
                    type="text" 
                    className="w-full px-4 py-3 bg-slate-50 border-none rounded-xl text-sm font-medium focus:ring-2 focus:ring-[#234] outline-none shadow-inner"
                    value={formData.unit}
                ...
                <div className="space-y-1.5">
                  ...
                  <input 
                    type="number" 
                    className="w-full px-4 py-3 bg-rose-50/30 border-none rounded-xl text-sm font-black text-rose-600 focus:ring-2 focus:ring-rose-200 outline-none shadow-inner"
              ...
              <div className="space-y-1.5 text-left">
                <div className="flex items-center justify-between pl-1 border-b border-indigo-100 pb-1">
                  <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">판매 단가 *</label>
                  <button 
                    type="button"
                ...
                <div className="relative">
                  <input 
                    type="number" 
                    className="w-full px-4 py-3 bg-indigo-50/30 border-none rounded-xl text-sm font-black text-indigo-700 focus:ring-2 focus:ring-indigo-200 outline-none shadow-inner"
                  ...
                  {formData.purchasePrice && formData.sellingPrice && (
                    <div className="absolute right-4 top-1/2 -translate-y-1/2 text-[10px] font-black text-emerald-500 bg-emerald-50 px-2 py-1 rounded-full border border-emerald-100">
                      MARG. {calculateMargin(parseInt(formData.sellingPrice), parseInt(formData.purchasePrice), formData.isSalesVatIncl, formData.isPurchaseVatIncl).marginRate.toFixed(1)}%
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="space-y-1.5 text-left">
                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest pl-1">비고</label>
                <textarea 
                  className="w-full px-4 py-3 bg-slate-50 border-none rounded-xl text-sm font-medium focus:ring-2 focus:ring-[#2D336B]/20 outline-none h-20 resize-none font-bold shadow-inner"
              ...
              <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 flex items-start gap-3">
                ...
                <p className="text-[10px] text-slate-500 leading-relaxed font-medium">
                  데이터는 실시간으로 클라우드에 연동됩니다. 원가 민감 정보는 암호화되어 보호됩니다.
                </p>
              </div>

              <button 
                type="submit"
                className="w-full py-4 bg-[#2D336B] text-white font-black rounded-2xl shadow-xl shadow-[#2D336B]/20 hover:scale-[1.01] active:scale-[0.99] transition-all mt-4 flex items-center justify-center gap-2"
              >
                <Check size={20} />
                {editingProductId ? '수정 내용 저장하기' : '데이터 등록 완료'}
              </button>
            </form>
          </ModalWrapper>
        )}
